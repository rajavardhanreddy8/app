import React, { useEffect, useState } from "react";
import axios from "axios";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Separator } from "@/components/ui/separator";
import { Loader2, Beaker, AlertCircle, CheckCircle2, History, ChevronDown, ChevronUp } from "lucide-react";
import SmilesInput from "../components/SmilesInput";
import RouteCard from "../components/RouteCard";
import useSynthesisStore from "../store/synthesisStore";

const BACKEND_URL = process.env.REACT_APP_BACKEND_URL;
const API = `${BACKEND_URL}/api`;

const DEVELOPMENT_STAGE_OPTIONS = [
  { value: "discovery", label: "Discovery", scale: "lab", defaultBatchSize: 0.05 },
  { value: "phase_1", label: "Phase I", scale: "lab", defaultBatchSize: 0.5 },
  { value: "phase_2", label: "Phase II", scale: "pilot", defaultBatchSize: 5 },
  { value: "phase_3", label: "Phase III", scale: "pilot", defaultBatchSize: 50 },
  { value: "commercial", label: "Commercial", scale: "industrial", defaultBatchSize: 100 },
];

const getStageConfig = (stage) =>
  DEVELOPMENT_STAGE_OPTIONS.find((option) => option.value === stage) || DEVELOPMENT_STAGE_OPTIONS[0];

const SynthesisPlannerPage = () => {
  // Store shared across pages
  const {
    targetSmiles, setTargetSmiles,
    optimizeFor,  setOptimizeFor,
    setPlannedRoutes,
    setPlanning,
    planningHistory, clearHistory,
  } = useSynthesisStore();

  // Local UI state
  const [maxSteps, setMaxSteps] = useState(5);
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState(null);
  const [error, setError] = useState(null);
  const [validationResult, setValidationResult] = useState(null);
  const [showHistory, setShowHistory] = useState(false);
  const [llmStatus, setLlmStatus] = useState(null);
  const [readiness, setReadiness] = useState(null);

  // Phase 4: Advanced mode state
  const [useAdvanced, setUseAdvanced] = useState(false);
  const [developmentStage, setDevelopmentStage] = useState("phase_2");
  const [batchSize, setBatchSize] = useState(getStageConfig("phase_2").defaultBatchSize);
  const [useIndustrialGate, setUseIndustrialGate] = useState(true); // Phase 11
  const [expandedRoute, setExpandedRoute] = useState(null);

  useEffect(() => {
    axios.get(`${API}/llm/status`)
      .then((response) => setLlmStatus(response.data))
      .catch(() => setLlmStatus(null));
    axios.get(`${API}/system/readiness`)
      .then((response) => setReadiness(response.data))
      .catch(() => setReadiness(null));
  }, []);

  const validateMolecule = async (smiles) => {
    if (!smiles) return;
    
    try {
      const response = await axios.post(`${API}/molecule/validate`, { smiles });
      setValidationResult(response.data);
      
      if (!response.data.valid) {
        setError(response.data.reason || "Invalid SMILES");
      } else {
        setError(null);
      }
    } catch (e) {
      console.error("Validation error:", e);
      setValidationResult({ valid: false, reason: "Validation failed" });
    }
  };

  const handlePlanSynthesis = async () => {
    if (!targetSmiles) {
      setError("Please enter a target SMILES");
      return;
    }

    setLoading(true);
    setPlanning(true);
    setError(null);
    setResult(null);

    try {
      const selectedStage = getStageConfig(developmentStage);
      const scale = selectedStage.scale;
      const params = useAdvanced
        ? `?use_advanced=true&scale=${scale}&batch_size_kg=${batchSize}&use_industrial_gate=${useIndustrialGate}`
        : '';

      const response = await axios.post(`${API}/synthesis/plan${params}`, {
        target_smiles: targetSmiles,
        max_steps: maxSteps,
        optimize_for: optimizeFor,
        stage: developmentStage,
        scale,
        batch_size_kg: batchSize,
      });

      setResult(response.data);
      // Persist to global store so RouteOptimizer can pick up the latest result.
      setPlannedRoutes(response.data.routes ?? []);
    } catch (e) {
      console.error("Planning error:", e);
      setError(
        e.response?.data?.detail || "Failed to generate synthesis plan. Please check your input."
      );
    } finally {
      setLoading(false);
      setPlanning(false);
    }
  };

  const exampleMolecules = [
    { name: "Aspirin", smiles: "CC(=O)Oc1ccccc1C(=O)O" },
    { name: "Caffeine", smiles: "CN1C=NC2=C1C(=O)N(C(=O)N2C)C" },
    { name: "Ibuprofen", smiles: "CC(C)Cc1ccc(cc1)C(C)C(=O)O" },
    { name: "Paracetamol", smiles: "CC(=O)Nc1ccc(O)cc1" },
  ];

  return (
    <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex items-center gap-3 mb-8">
          <Beaker className="w-8 h-8 text-purple-400" />
          <div>
            <h1 className="text-3xl font-bold text-white">
              AI Synthesis Planner
            </h1>
            <p className="text-purple-200/70 text-sm">
              Plan routes with cloud LLM fallback, RDKit, and process checks
            </p>
          </div>
        </div>

        {llmStatus && (
          <div className="mb-6 flex flex-wrap items-center gap-2 rounded-lg border border-purple-500/25 bg-white/5 px-4 py-3 text-xs text-purple-100">
            <Badge className={llmStatus.health === "live" ? "bg-emerald-500/20 text-emerald-200 border-emerald-400/30" : "bg-yellow-500/20 text-yellow-200 border-yellow-400/30"}>
              LLM {llmStatus.health}
            </Badge>
            <span>{llmStatus.provider}</span>
            {llmStatus.planner_models?.[0] && <span>Planner: {llmStatus.planner_models[0]}</span>}
            {llmStatus.validation?.ok === false && <span className="text-yellow-200">Config warning: check model validation</span>}
          </div>
        )}

        {readiness && (
          <div className="mb-6 rounded-lg border border-slate-500/25 bg-white/5 px-4 py-3 text-xs text-slate-100">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <Badge className={readiness.research_grade_ready ? "bg-emerald-500/20 text-emerald-200 border-emerald-400/30" : "bg-yellow-500/20 text-yellow-200 border-yellow-400/30"}>
                Research readiness {readiness.research_grade_ready ? "ready" : "limited"}
              </Badge>
              <Badge className="bg-slate-500/20 text-slate-200 border-slate-400/30">
                DB {readiness.database?.mode}
              </Badge>
              <Badge className="bg-slate-500/20 text-slate-200 border-slate-400/30">
                Auth {readiness.auth?.mode}
              </Badge>
              <Badge className="bg-slate-500/20 text-slate-200 border-slate-400/30">
                Yield {readiness.models?.yield_predictor?.fallback_state}
              </Badge>
            </div>
            {(readiness.blockers?.length > 0 || readiness.warnings?.length > 0) && (
              <div className="text-yellow-100">
                {[...(readiness.blockers || []), ...(readiness.warnings || [])].slice(0, 2).join(" ")}
              </div>
            )}
          </div>
        )}

        {/* Input Section */}
        <Card className="mb-8 bg-white/10 backdrop-blur-md border-purple-500/30">
          <CardHeader>
            <CardTitle className="text-white">Target Molecule</CardTitle>
            <CardDescription className="text-purple-200">
              Enter the SMILES notation of your target molecule
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* SMILES Input with live 2D preview */}
            <SmilesInput
              id="smiles-input"
              label="SMILES String"
              value={targetSmiles}
              onChange={(v) => { setTargetSmiles(v); setValidationResult(null); }}
              placeholder="e.g., CC(=O)Oc1ccccc1C(=O)O"
              previewSize={200}
            />

            {/* Server-side validation result (shown after "Validate" or on blur) */}
            {validationResult && (
              <div className="mt-1">
                {validationResult.valid ? (
                  <Alert className="bg-green-500/20 border-green-500/50">
                    <CheckCircle2 className="h-4 w-4 text-green-400" />
                    <AlertDescription className="text-green-200">Valid SMILES structure</AlertDescription>
                  </Alert>
                ) : (
                  <Alert className="bg-red-500/20 border-red-500/50">
                    <AlertCircle className="h-4 w-4 text-red-400" />
                    <AlertDescription className="text-red-200">
                      {validationResult.reason || "Invalid SMILES"}
                    </AlertDescription>
                  </Alert>
                )}
              </div>
            )}


            <Separator className="bg-purple-500/30" />

            {/* Phase 4: Advanced Mode Toggle */}
            <div className="p-4 bg-purple-900/20 rounded-lg border border-purple-500/30">
              <div className="flex items-center justify-between mb-3">
                <label className="text-white font-medium">
                  Advanced Mode (Industrial Optimization)
                </label>
                <button
                  onClick={() => setUseAdvanced(!useAdvanced)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    useAdvanced ? 'bg-purple-600' : 'bg-gray-600'
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      useAdvanced ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
              </div>
              
              {useAdvanced && (
                <div className="space-y-3 pt-3 border-t border-purple-500/30">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label
                        className="block text-xs text-purple-200 mb-1"
                        title="Affects route prioritization and minimum acceptable yield thresholds"
                      >
                        Development Stage
                      </label>
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-1">
                        {DEVELOPMENT_STAGE_OPTIONS.map((stageOption) => (
                          <button
                            key={stageOption.value}
                            type="button"
                            title="Affects route prioritization and minimum acceptable yield thresholds"
                            onClick={() => {
                              setDevelopmentStage(stageOption.value);
                              setBatchSize(getStageConfig(stageOption.value).defaultBatchSize);
                            }}
                            className={`rounded border px-2 py-2 text-xs transition ${
                              developmentStage === stageOption.value
                                ? "border-purple-300 bg-purple-600 text-white"
                                : "border-purple-500/30 bg-purple-900/30 text-purple-200"
                            }`}
                          >
                            {stageOption.label}
                          </button>
                        ))}
                      </div>
                      <p className="text-[11px] text-purple-300 mt-1">
                        Maps to {getStageConfig(developmentStage).scale} scale
                      </p>
                    </div>
                    
                    <div>
                      <label className="block text-xs text-purple-200 mb-1">Batch Size (kg)</label>
                      <Input
                        type="number"
                        value={batchSize}
                        onChange={(e) => setBatchSize(parseFloat(e.target.value))}
                        step={getStageConfig(developmentStage).scale === 'lab' ? 0.1 : getStageConfig(developmentStage).scale === 'pilot' ? 1 : 10}
                        min={getStageConfig(developmentStage).scale === 'lab' ? 0.001 : getStageConfig(developmentStage).scale === 'pilot' ? 1 : 100}
                        className="bg-purple-900/40 border-purple-500/30 text-white text-sm"
                      />
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-3 bg-slate-900/40 p-2 rounded border border-purple-500/20">
                    <label className="text-xs text-purple-200 cursor-pointer" onClick={() => setUseIndustrialGate(!useIndustrialGate)}>
                      Enable Industrial Acceptability and AR&D Gate
                    </label>
                    <button
                      onClick={() => setUseIndustrialGate(!useIndustrialGate)}
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                        useIndustrialGate ? 'bg-purple-600' : 'bg-gray-600'
                      }`}
                    >
                      <span
                        className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${
                          useIndustrialGate ? 'translate-x-5' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                  <p className="text-xs text-purple-300">
                    Uses ML retrosynthesis, scale optimization, and industrial cost modeling
                  </p>
                </div>
              )}
            </div>

            {/* Options */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-white font-medium mb-2 block">
                  Max Steps: {maxSteps}
                </label>
                <Input
                  type="range"
                  min="1"
                  max="10"
                  value={maxSteps}
                  onChange={(e) => setMaxSteps(parseInt(e.target.value))}
                  className="bg-white/20"
                />
              </div>

              <div>
                <label className="text-white font-medium mb-2 block">
                  Optimize For
                </label>
                <select
                  data-testid="optimize-select"
                  value={optimizeFor}
                  onChange={(e) => setOptimizeFor(e.target.value)}
                  className="w-full p-2 rounded-md bg-white/20 border border-purple-300/50 text-white"
                >
                  <option value="balanced" className="bg-slate-800">Balanced</option>
                  <option value="yield" className="bg-slate-800">High Yield</option>
                  <option value="cost" className="bg-slate-800">Low Cost</option>
                  <option value="time" className="bg-slate-800">Fast (Few Steps)</option>
                  <option value="green" className="bg-slate-800">Green Chemistry</option>
                  <option value="regulatory" className="bg-slate-800">Regulatory Ready</option>
                </select>
              </div>
            </div>

            {/* Submit Button */}
            <Button
              data-testid="plan-button"
              onClick={handlePlanSynthesis}
              disabled={loading || !targetSmiles}
              className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold py-6 text-lg"
            >
              {loading ? (
                <>
                  <Loader2 className="mr-2 h-5 w-5 animate-spin" />
                  Planning Synthesis Routes...
                </>
              ) : (
                "Generate Synthesis Plan"
              )}
            </Button>

            {/* Recent sessions history panel */}
            {planningHistory.length > 0 && (
              <div>
                <button
                  onClick={() => setShowHistory(!showHistory)}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 6,
                    background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)',
                    borderRadius: 8, padding: '6px 14px', color: '#c4b5fd',
                    fontSize: 13, cursor: 'pointer', width: '100%',
                  }}
                >
                  <History size={14} />
                  <span>{planningHistory.length} recent session{planningHistory.length > 1 ? 's' : ''}</span>
                  <span style={{ marginLeft: 'auto' }}>
                    {showHistory ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                  </span>
                </button>

                {showHistory && (
                  <div style={{
                    marginTop: 8, border: '1px solid rgba(139,92,246,0.2)',
                    borderRadius: 8, overflow: 'hidden',
                  }}>
                    {planningHistory.map((session, i) => (
                      <div
                        key={i}
                        onClick={() => {
                          setTargetSmiles(session.target);
                          setValidationResult(null);
                        }}
                        style={{
                          display: 'flex', alignItems: 'center', gap: 10,
                          padding: '8px 12px',
                          background: i % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'transparent',
                          borderBottom: i < planningHistory.length - 1 ? '1px solid rgba(139,92,246,0.1)' : 'none',
                          cursor: 'pointer',
                          transition: 'background 0.15s',
                        }}
                        onMouseEnter={e => e.currentTarget.style.background = 'rgba(139,92,246,0.15)'}
                        onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'transparent'}
                      >
                        <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#e9d5ff', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {session.target}
                        </span>
                        <span style={{ fontSize: 11, color: '#a78bfa', whiteSpace: 'nowrap' }}>
                          {session.routeCount} route{session.routeCount !== 1 ? 's' : ''}
                        </span>
                        <span style={{ fontSize: 10, color: '#7c3aed', whiteSpace: 'nowrap' }}>
                          {new Date(session.timestamp).toLocaleDateString()}
                        </span>
                      </div>
                    ))}
                    <div style={{ padding: '6px 12px', textAlign: 'right' }}>
                      <button
                        onClick={(e) => { e.stopPropagation(); clearHistory(); setShowHistory(false); }}
                        style={{ fontSize: 11, color: '#f87171', background: 'none', border: 'none', cursor: 'pointer' }}
                      >
                        Clear history
                      </button>
                    </div>
                  </div>
                )}
              </div>
            )}
          </CardContent>
        </Card>

        {/* Error Display */}
        {error && (
          <Alert className="mb-8 bg-red-500/20 border-red-500/50">
            <AlertCircle className="h-4 w-4 text-red-400" />
            <AlertDescription className="text-red-200">{error}</AlertDescription>
          </Alert>
        )}

        {/* Results Display */}
        {result && result.routes && result.routes.length > 0 && (
          <div className="space-y-6" data-testid="results-section">
            {/* Summary */}
            <Card className="bg-white/10 backdrop-blur-md border-purple-500/30">
              <CardHeader>
                <CardTitle className="text-white">Results Summary</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  <div>
                    <p className="text-purple-300 text-sm">Routes Found</p>
                    <p className="text-3xl font-bold text-white">{result.routes.length}</p>
                  </div>
                  <div>
                    <p className="text-purple-300 text-sm">Computation Time</p>
                    <p className="text-3xl font-bold text-white">{result.computation_time_seconds}s</p>
                  </div>
                  <div>
                    <p className="text-purple-300 text-sm">Tokens Used</p>
                    <p className="text-3xl font-bold text-white">{result.tokens_used}</p>
                  </div>
                  <div>
                    <p className="text-purple-300 text-sm">Target</p>
                    <p className="text-sm font-mono text-white truncate">{result.target_smiles}</p>
                  </div>
                </div>
              </CardContent>
            </Card>

            {/* Routes */}
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-white">Synthesis Routes</h2>
              
              {result.metadata?.grouped_routes ? (
                <div className="space-y-8">
                  {result.metadata.grouped_routes.accepted_routes?.length > 0 && (
                    <div className="space-y-4">
                      <h3 className="text-xl font-semibold text-green-400 border-b border-green-500/30 pb-2">✅ Accepted for Production</h3>
                      {result.metadata.grouped_routes.accepted_routes.map((route, routeIdx) => (
                        <RouteCard key={`acc-${route.id || routeIdx}`} route={route} routeIdx={routeIdx} />
                      ))}
                    </div>
                  )}

                  {result.metadata.grouped_routes.ard_required_routes?.length > 0 && (
                    <div className="space-y-4">
                      <h3 className="text-xl font-semibold text-yellow-400 border-b border-yellow-500/30 pb-2">🧪 AR&D Optimization Required</h3>
                      {result.metadata.grouped_routes.ard_required_routes.map((route, routeIdx) => (
                        <RouteCard key={`ard-${route.id || routeIdx}`} route={route} routeIdx={routeIdx} />
                      ))}
                    </div>
                  )}

                  {result.metadata.grouped_routes.rejected_routes?.length > 0 && (
                    <div className="space-y-4 opacity-80">
                      <h3 className="text-xl font-semibold text-red-400 border-b border-red-500/30 pb-2">❌ Rejected Routes</h3>
                      {result.metadata.grouped_routes.rejected_routes.map((route, routeIdx) => (
                        <RouteCard key={`rej-${route.id || routeIdx}`} route={route} routeIdx={routeIdx} />
                      ))}
                    </div>
                  )}

                  {result.metadata.grouped_routes.exploratory_routes?.length > 0 && (
                    <div className="space-y-4 opacity-70">
                      <h3 className="text-xl font-semibold text-gray-400 border-b border-gray-500/30 pb-2">🔬 Exploratory Only</h3>
                      {result.metadata.grouped_routes.exploratory_routes.map((route, routeIdx) => (
                        <RouteCard key={`exp-${route.id || routeIdx}`} route={route} routeIdx={routeIdx} />
                      ))}
                    </div>
                  )}
                </div>
              ) : (
                <div className="space-y-4">
                  {result.routes.map((route, routeIdx) => (
                    <RouteCard key={route.id || routeIdx} route={route} routeIdx={routeIdx} />
                  ))}
                </div>
              )}
            </div>
          </div>
        )}
        {result && result.routes && result.routes.length === 0 && (
          <Alert className="bg-yellow-500/20 border-yellow-500/50">
            <AlertCircle className="h-4 w-4 text-yellow-400" />
            <AlertDescription className="text-yellow-200">
              No synthesis routes found. The molecule may be too complex or require specialized reactions.
            </AlertDescription>
          </Alert>
        )}
    </div>
  );
};

export default SynthesisPlannerPage;


