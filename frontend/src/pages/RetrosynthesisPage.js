...
const RetrosynthesisPage = () => {
  ...
  const planRetrosynthesis = async (smilesInput) => {
    ...
    try {
      ...
      setError(
        e.response?.data?.detail || "Failed to generate retrosynthesis routes."
      );
    } finally {
      setLoading(false);
    }
  ...
  const renderRouteTree = (route, depth = 0) => {
          ...
          <div>
            ...
            <div className="flex gap-2">
              <Input
                ...
                onKeyDown={(e) => e.key === "Enter" && planRetrosynthesis()}
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-white">
                  Found {result.num_routes || result.routes?.length || 0} Routes
                </CardTitle>
