...
try:
    ...
    # Fallback for search during implementation
    class RetrosynthesisEngine: pass
    REACTION_TEMPLATES = []
...
class TestFindPrecursors:
    ...
    def test_caching_returns_same_result(self, engine):
        smi = "CC(=O)Oc1ccccc1"
        if not hasattr(engine, '_find_precursors'): pytest.skip("Not implemented")
...
class TestCalculateNodeScore:
    ...
    def test_building_block_scores_higher_than_nonbuilding_block(self, engine):
        ...
        if hasattr(engine, '_is_building_block') and engine._is_building_block("CCO"):
    ...
    def test_building_block_bonus_is_positive(self, engine):
        ...
        if hasattr(engine, '_is_building_block') and engine._is_building_block(bb):
...
class TestSearchRoutesIntegration:
    ...
    def test_invalid_smiles_returns_empty(self, engine):
        if not hasattr(engine, 'search_routes'): pytest.skip("Not implemented")
