...
class ConditionPredictor:
    ...
    def __init__(self, model_dir: str = "/app/backend/models"):
        ...
        # Label encoders
        self.catalyst_encoder = LabelEncoder()
    ...
    def predict_conditions(self, reaction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not all([self.temp_model, self.catalyst_model, self.solvent_model]):
            logger.warning("Models not available")
            return {
        ...
        try:
            ...
            # Overall confidence
            confidence = 'high' if min(cat_confidence, solv_confidence) > 0.7 else \
                        'medium' if min(cat_confidence, solv_confidence) > 0.5 else 'low'
    ...
    def _get_alternatives(self, proba: np.ndarray, encoder: LabelEncoder, top_k: int = 3) -> List[Dict]:
...
async def main():
    ...
    if metrics:
        ...
        conditions = predictor.predict_conditions(test_reaction)
