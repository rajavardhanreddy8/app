...
class RetrosynthesisEngine:
    ...
    def _extract_route(self, leaf_node: ReactionNode) -> Optional[Dict]:
        ...
        for step in steps:
            step.update({
                'estimated_yield': step_yield,
                'estimated_yield_percent': step_yield,
                'estimated_cost_usd': step_cost,
                'prediction_source': 'rules',
                'confidence': 0.45,
                'uncertainty': {'yield_percent': [-20.0, 20.0], 'cost_usd': [-25.0, 75.0]},
                'warnings': ['Rule-based retrosynthesis estimate; verify chemistry and conditions manually.'],
                'review_status': 'needs_human_review',
                'human_review_required': True,
                'evidence': {
                    'source': 'retrosynthesis_template_rules',
                    'method': 'SMARTS template search with heuristic economics',
                    'model_version': 'retrosynthesis_rules_v1',
                    'confidence': 0.45,
                },
            })
        estimated_yield = round((step_yield / 100.0) ** total_steps * 100.0, 2)
        ...
        estimated_cost = round((step_cost * total_steps) + (10.0 * len(clean_sm)), 2)
        ...
        return {
            ...
            'confidence': 0.45,
            'uncertainty': {'yield_percent': [-20.0, 20.0], 'cost_usd': [-25.0, 75.0]},
            'warnings': ['Route generated by template rules; all economics are heuristic estimates.'],
            'review_status': 'needs_human_review',
            'human_review_required': True,
            'evidence': {
                'source': 'retrosynthesis_template_rules',
